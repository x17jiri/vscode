...
export class Cursor {
	...
	constructor(context: CursorContext) {
		...
		this._setState(
			context,
			new SingleCursorState(new Range(1, 1, 1, 1), SelectionStartKind.Simple, new Position(1, 1)),
			new SingleCursorState(new Range(1, 1, 1, 1), SelectionStartKind.Simple, new Position(1, 1))
		);
	}
	...
	private _setState(context: CursorContext, modelState: SingleCursorState | null, viewState: SingleCursorState | null): void {
		...
		if (!modelState) {
			...
			modelState = new SingleCursorState(selectionStart, viewState.selectionStartKind, position);
		} else {
			...
			modelState = new SingleCursorState(selectionStart, modelState.selectionStartKind, position);
		}
		...
		if (!viewState) {
			...
			viewState = new SingleCursorState(viewSelectionStart, modelState.selectionStartKind, viewPosition);
		} else {
