(function () {
  ...
  module.exports = function (exe, options, callback) {
    ...
    console.error("spawning: ", rcedit, ", ", args);
    child = spawn(rcedit, args);
    stderr = '';
    error = null;
    child.on('error', function (err) {
      log.error("sparning rcedit returned: error ", err);
      return error != null ? error : error = err;
    });
    child.stderr.on('data', function (data) {
      log.error("sparning rcedit returned: data ", data);
      return stderr += data;
    });
    return child.on('close', function (code) {
      log.error("sparning rcedit returned: close: code = ", code, ", error = ", error);
      if (error != null) {
        log.error("sparning rcedit br 1");
